...
### 0. 局域网访问:其他设备打不开 / 提示"连接不安全"

#### 症状

- 本机 `http://localhost:7860` 正常,同 WiFi 的手机/另一台电脑打不开
- 或浏览器提示"此站点的连接不安全" / "您的连接不是私密连接"

#### 排查(按序)

1. **确认链接带 `http://` 前缀**:很多手机浏览器输入 IP 时会默认尝试
   `https://`,而本服务是纯 HTTP,会被升级成 https 后报证书错误。
   ✅ 正确:`http://192.168.x.x:7860` ❌ 错误:`https://192.168.x.x:7860`
2. **确认服务器监听的是 0.0.0.0**:
   `ss -tlnp | grep 7860` 应显示 `0.0.0.0:7860`(start.sh 启动即满足)。
3. **确认防火墙放行**:
   ```bash
   sudo ufw status          # Ubuntu;inactive 即无拦截
   sudo ufw allow 7860/tcp  # 若 active 则放行
   ```
4. **确认两设备真在同一网络**:有的路由器开了"AP 隔离/访客网络",设备间
   互相不可见。用另一台设备 ping 本机 IP 验证:`ping 192.168.x.x`。
5. **跨网段/子网掩码问题**:本机若是 `/23` 大子网而手机在 `/24`,一般也通;
   不通时看路由器 DHCP 设置。
