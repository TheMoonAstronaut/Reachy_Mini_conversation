...
#### P9 — 部署形态扩展(开源后,调研已完成 2026-09-18)

背景:无线版机身内置 CM4(4GB,WiFi)自治,PC 经网络控制;Lite 版 USB-C
作设备口直连 PC(本项目有线模式)。无线版硬件的 USB-C 实测支持双模
(插入 PC 即枚举相机/声卡/电机串口,有线模式全功能可用)。

- ~~P9.1 无线模式补齐相机/手部跟随~~(**已废弃,2026-09-20 产品范围决策**:无线版只在树莓派本体跑,PC 不做无线远程操控;原方案存档):
  无线 client 构造补 `media_backend=MediaBackend.WEBRTC`(SDK 远程客户端
  一等路径,daemon 永远持有物理相机);取流回退链"本地 USB → 远程媒体";
  `/camera_feed` 同回退。MediaPipe 仍在 PC 跑,端到端 ~100-200ms、~10Hz
  可用。落地后去掉下拉"暂不支持相机/手部跟随"标注。
- **P9.2 on-robot 裁剪模式**(中,代码已落地 2026-09-18,待树莓派真机验收):app 直接跑在 CM4 上 —— 砍 Mujoco sim
  依赖(物理+仿真视频流是 CM4 扛不动的部分),`ws/state` 数据源换真机
  (3D 视图是浏览器渲染,保留,CM4 零负担),相机 USB 本地直读,手部跟随
  降级(5-8fps)或浏览器端 JS MediaPipe(官方 hand_tracker 同款)。
